...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        # Dynamically select up to 5 replicate columns starting from column 1
        replicate_cols = df_input.columns[1:min(6, len(df_input.columns))]

        # Flatten replicate data points for statistical calculations
        ...
        # FIX FOR PANDAS 2.0+: Convert columns to object type before assigning text
        ...
        if overall_status == "FAIL!":
            ...
            st.success("✅ Success: All raw data points are perfectly within limits.")
        ...
        # Function to highlight [FAIL] and [PASS] cells
        ...
        # Using map() with fallback to applymap() for backward compatibility
        if hasattr(df_result.style, "map"):
            styled_df = df_result.style.map(highlight_failures, subset=replicate_cols)
        else:
            styled_df = df_result.style.applymap(highlight_failures, subset=replicate_cols)
